...
static int default_burst_for_type(ShipType type) {
    switch (type) {
        case PATROL:
            return 3;
        case FISHING:
...
static int default_priority_for_type(ShipType type) {
    switch (type) {
        case PATROL:
            return 1;
        case FISHING:
...
static int deadline_increment_for_type(ShipType type) {
    switch (type) {
        case PATROL:
            return 5;
        case FISHING:
...
static void reorder_queue_by_scheduler(ReadyQueue *queue, SchedulerType scheduler) {
    ...
    while (current != NULL) {
        ...
        free(current);
        enqueue_ordered(queue, ship, scheduler);
...
static void process_pending_ship_requests(
    ...
    while (xQueueReceive(ship_request_queue, &request, 0) == pdTRUE) {
        ...
        int burst_time = default_burst_for_type(request.type);
        int priority = default_priority_for_type(request.type);

        next_dynamic_deadline += deadline_increment_for_type(request.type);
        ...
        if (!enqueue_ordered(target_queue, ship, scheduler)) {
        ...
        printf("[NUEVO BARCO] %s agregado a %s. Se reorganiza la cola con el calendarizador actual.\n",
               ship->name,
               target_queue->name);
